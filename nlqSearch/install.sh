...
if [ -z "${GROQ_API_KEY:-}" ] && [ -z "${GEMINI_API_KEY:-}" ]; then
  echo ""
  echo "WARNING: Neither GROQ_API_KEY nor GEMINI_API_KEY is set."
  echo "  The plugin will default to the 'ollama' backend."
  echo "  To use Groq (recommended), set GROQ_API_KEY before running:"
  echo "    export GROQ_API_KEY=your-key && sudo -E bash install.sh"
...
cp "${BUILD_DIR}/server/llm_backends/groq.js"        "${INSTALL_DIR}/server/llm_backends/"
...
echo ""
if [ -z "${GROQ_API_KEY:-}" ] && [ -z "${GEMINI_API_KEY:-}" ]; then
  echo "NOTE: Set GROQ_API_KEY and re-run install.sh to enable the Groq backend."
  echo "  export GROQ_API_KEY=your-key && sudo -E bash install.sh"
fi
